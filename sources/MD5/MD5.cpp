...
namespace RFC1321
{
	...
	MD5::MD5(char* input, uint32_t length)
	{
		init();
		update(input, length);
		finalize();
	...
	void MD5::transform(const uint8_t block[MD5_BLOCKSIZE])
	{
	...
	void MD5::update(const unsigned char input[], uint32_t length)
	{
		...
		uint32_t index = _count[0] / 8 % MD5_BLOCKSIZE;
		...
		uint32_t firstpart = MD5_BLOCKSIZE - index;
		uint32_t i;
