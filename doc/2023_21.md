...
After 327 steps diamond pattern becomes visible and some numbers start to repeat. The number of points in the original map dropped down to `7523`, but four adjacent maps got `7523` points now. It seems that diamond pattern grows from the center to outside:
...
And the result is `618261433219147`, which is correct for my input :)
