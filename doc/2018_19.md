...
__First part__ of the puzzle can be solved by taking excerpts of source code already written for 2018/16 ([`.cpp`](https://github.com/tbielak/AoC_cpp/blob/master/sources/2018/2018_16.cpp) [`.h`](https://github.com/tbielak/AoC_cpp/blob/master/sources/2018/2018_16.h)) + writting some additional code. Let's do this here, just for fun!
...
To solve __part two__ of the puzzle we should set `reg[0]` to `1` before executing the program. Unfortunatelly, in such case the execution seems to be running infinitely. We need to analyse the program provided on input.
...
Now, the value of `R0` is checked in fancy way: if `R0 == 0` (__first part__ of puzzle) instruction 25 does nothing and then instruction 26 is executed, meaning `goto 1`; if `R0 == 1` (__part two__ of the puzzle) instruction 26 is skipped and instructions 27-34 are executed before `goto 1` in instruction 35.

Instructions 27-32 calculate new value of `R5`, which is added to `R2` by instruction 33. Finally `R2 == 10551418` for my input. Before `goto 1` register `R0` is set to zero:
...
Observations/conslusions, so far:
...
OK, but what really does this algorithm?
...
__Looks__ like the solution, __but__ there are various inputs for the puzzle!
...
- use faster substitute of algorithm stored originally in input in instructions 1-16.
