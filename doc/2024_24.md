...
AoC Puzzle: [`Step Counter`](https://adventofcode.com/2024/day/24)
...
- The first gate `x00 XOR y00 -> z00` adds bits #0 of `x` and `y` and store the result in bit #0 of `z`.
- Second gate `x00 AND y00 -> gwq` checks if there was a carry on addition of bits #0. The carry flag is put in output wire `gwq`.
...
So, in __part two__ we need to find __eight output wires swapped__. So we need to check if outputs of the gates are correctly connected to the inputs of the other gates.
...
Let's prepare the code which solves it. First: a skeleton of the loop which will checks the bits:
...
We'are starting from bit 1, because handling bit 0 is different from handling next bits. Let's assume that bit 0 is calculated correctly. Thus the only task related to bit 0 is finding `carry` wire, which is and output of `x00 AND y00` gate:
...
Method `find_output` returns name of the output wire or empty string if the gate speficied by two inputs and type was not found in input.
...
It does the swap, but it also registers wires which were swapped in `_swapped` class member defined as:
...
Finally, we need to create the result of __part two__ by joining already sorted names of swapped wires. Let's put the whole code into `eight_wires` method:
